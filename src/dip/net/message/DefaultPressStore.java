...
public class DefaultPressStore implements PressStore {
    ...
    protected final List<PressMessage> messages;

    // transient fields
    /** Create a DefaultPressStore */
    public DefaultPressStore() {
        messages = new ArrayList<PressMessage>(100);
    }// DefaultPressStore()
	...
	public synchronized void storeMessage(PressMessage pm) {
        ...
        ListIterator<PressMessage> iter = messages.listIterator();
        while (iter.hasNext()) {
            PressMessage msg = iter.next();
            if (compare(pm, msg)) {
