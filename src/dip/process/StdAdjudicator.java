...
public class StdAdjudicator implements Adjudicator {
    ...
    private void adjudicateMoves() {
        ...
        final List<OrderState> osList = new ArrayList<OrderState>(orderList.size());

        for(final Orderable orderable: orderList) {
            OrderState os = new OrderState(orderable);
            ...
            if (position.hasUnit(province)) {
                ...
                addResult(new OrderResult(orderable, Utils.getLocalString(STDADJ_MV_NO_UNIT)));
            }
        }
    ...
    private void adjudicateRetreats() {
        ...
        for(final Orderable order: orderList) {
     ...
     */
    private void createRemoveOrders(final List<OrderState> osList, Power power, int ordersToMake) {
        ...
        for (int i = 0; i < ordersToMake; i++) {
            final LinkedList<Province> ties = new LinkedList<Province>();
            int maxDist = 0;
            ...
            if (ties.size() == 0) {
                ...
                for(final Province province: ties) {
                    Unit unit = position.getUnit(province);
    ...
    private void createDisbandOrder(final List<OrderState> osList, Province province) {
        Unit unit = position.getUnit(province);
     ...
     *
     */
    private List<Move> findMovesTo(Province moveDest) {
        final List<Move> list = new ArrayList<Move>(6);
    ...
    private List<OrderState> findOrderStatesForPower(Power power) {
        final List<OrderState> list = new ArrayList<OrderState>(32);
    ...
    private List<OrderState> getConvoyList(Move move) {
        final List<OrderState> list = new ArrayList<OrderState>(8);

        for (final OrderState os: list) {
    ...
    private int markCircularMoves() {
        final List<OrderState> movesToCheck = new LinkedList<OrderState>();
        final LinkedList<OrderState> chain = new LinkedList<OrderState>();	// we'll be using this as a Stack; it will be cleared if chain is not complete.
        ...
        for (final OrderState moveOS : movesToCheck) {
            chain.clear();
            isChainCircular = false;

            if (!moveOS.isCircular() && moveOS.getOrder() instanceof Move) {
                ...
                while (nextMoveOS != null && !isChainCircular) {
                    if (!nextMoveOS.isCircular() && nextMoveOS.getOrder() instanceof Move) {
