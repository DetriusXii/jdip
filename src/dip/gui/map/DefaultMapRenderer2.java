...
public class DefaultMapRenderer2 extends MapRenderer2 {
    ...
    private void checkSymbols()
        ...
        final Map<Object, Node> map = SVGUtils.tagFinderSVG(Arrays.asList(SYMBOLS), doc.getRootElement());
        ...
        for (final String symbol: SYMBOLS) {
            if (mapMeta.getSymbolSize(symbol) == null) {
                throw new MapException("Missing required <jdipNS:SYMBOLSIZE> element for symbol name \"" + 
                        symbol + "\"");
            }
        }
    ...
    private void mapLayers()
            throws MapException {
        final List<Object> recastList = new ArrayList<Object>();
        for(final String layer: LAYERS) {
            recastList.add(layer);
        }
        SVGUtils.tagFinderSVG(layerMap, recastList, doc.getRootElement());
        for (final String layer: LAYERS) {
    ...
    private void addProvinceHilitesToTracker() {
        ...
        final List<Object> recastProvList = new ArrayList<Object>();
        for (final String uscoreProv : uscoreProvList) {
            recastProvList.add(uscoreProv);
        }
        Map<Object, Node> map = SVGUtils.tagFinderSVG(recastProvList, doc.getRootElement(), true);
        ...
        for (int i = 0; i < lookupProvList.size(); i++) {
            final Node node = map.get(uscoreProvList.get(i));

            final SVGElement element = node != null && node instanceof SVGElement
